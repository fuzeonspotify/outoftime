...
func prepare_library() -> void:
	if _library_is_ready or _preparing:
		return
	_preparing = true
...
func has_cached_audio() -> bool:
	return not _paths_by_pack.is_empty()
...
func _prepare_library() -> void:
	...
	for pack_index: int in range(pack_ids.size()):
		var pack_id: String = pack_ids[pack_index]
		var source_variant: Variant = PACKS.get(pack_id)
		if source_variant is Dictionary:
			var source: Dictionary = source_variant
			progress_changed.emit(
				float(pack_index) / float(maxi(1, pack_ids.size())),
				"PREPARING %s AUDIO" % pack_id.to_upper()
			)
	...
	progress_changed.emit(1.0, "HEADPHONE AUDIO READY")
	library_ready.emit()
...
func _ensure_pack(pack_id: String, source: Dictionary) -> void:
	...
	for candidate_url: String in candidates:
		...
		if not _write_bytes(archive_path, archive_bytes):
			continue
		var extracted: bool = await _extract_audio_archive(archive_path, pack_root, pack_id)
		if extracted:
			break
...
func _extract_audio_archive(archive_path: String, destination_root: String, pack_id: String) -> bool:
	...
	for internal_path: String in archive_files:
		...
		if not file_bytes.is_empty() and _write_bytes(destination_path, file_bytes):
			extracted_count += 1
		processed_count += 1
		if processed_count % EXTRACTION_BATCH_SIZE == 0:
			progress_changed.emit(0.5, "UNPACKING %s AUDIO  //  %d FILES" % [pack_id.to_upper(), extracted_count])
			await get_tree().process_frame
	reader.close()
