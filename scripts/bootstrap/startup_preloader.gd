...
func get_void_prototype(_model_path: String) -> Node3D:
	# Compatibility for older cached scenes. The active story now uses Memory Train.
	return null
...
func _prepare_all() -> void:
	...
	_used_fallbacks = not scenes_ready or not audio_ready or not level_ready
...
func _prepare_scene_resources() -> bool:
	...
	for scene_index: int in range(SCENE_PATHS.size()):
		...
		_update_progress(
			lerpf(0.04, 0.18, float(scene_index + 1) / float(SCENE_PATHS.size())),
...
func _prepare_audio() -> bool:
	_update_progress(0.22, "PREPARING HEADPHONE AUDIO")
...
func _prepare_level_assets() -> bool:
	_update_progress(0.46, "PREPARING CEMETERY AND NIGHTCLUB MODELS")
