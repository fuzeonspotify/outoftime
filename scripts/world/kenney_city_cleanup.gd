...
func _ready() -> void:
	call_deferred("_wait_for_kenney_city")
...
func _wait_for_kenney_city() -> void:
	...
	while elapsed_time < KENNEY_WAIT_TIMEOUT:
		...
		if kenney_container != null:
			...
			_face_kenney_buildings_toward_street(kenney_container)
			return
...
func _face_kenney_buildings_toward_street(kenney_container: Node3D) -> void:
	...
	for child: Node in model_children:
		...
		var corrected_yaw: float = snappedf(rad_to_deg(street_angle - front_angle), 90.0)
		model_root.rotation_degrees = Vector3(0.0, corrected_yaw, 0.0)
		model_root.set_meta("detected_facade_direction", detected_front)
...
func _detect_detailed_facade_direction(model_root: Node3D) -> Vector3:
	...
	var best_side: int = 0
	var best_score: float = side_scores[0]
	for side_index: int in range(1, side_scores.size()):
		if side_scores[side_index] > best_score:
			best_score = side_scores[side_index]
