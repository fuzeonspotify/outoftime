...
func _ready() -> void:
	_build_balcony_access.call_deferred()


func _build_balcony_access() -> void:
	var scene_root: Node3D = get_parent() as Node3D
	if scene_root == null:
		return

	_remove_original_stair_geometry(scene_root)
	_remove_route_clutter(scene_root)
	_build_clear_route(scene_root)


func _remove_original_stair_geometry(scene_root: Node3D) -> void:
...
func _remove_route_clutter(scene_root: Node3D) -> void:
...
func _build_clear_route(scene_root: Node3D) -> void:
	_create_static_box(
		scene_root,
		"RebuiltBalconyFloor",
